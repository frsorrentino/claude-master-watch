 ...
 */
export const UiCard: React.FC<{ w: number; name: string; age: string; text: string; badge?: string; icon?: "check" | "play"; light?: number; checkDraw?: number; body?: CardBody; bodyOpacity?: number }> = ({ w, name, age, text, badge = UI.badge, icon = "check", light = 0, checkDraw, body = CARD_BODY, bodyOpacity }) => (
  <div style={{ width: w, boxSizing: "border-box", padding: "24px 24px 24.5px", borderRadius: 42, background: UI.surface, color: UI.text, fontFamily: "Roboto",
    ...
    <div style={{ marginTop: body.shift, fontSize: body.size, lineHeight: `${body.line}px`, letterSpacing: -0.6, ...(bodyOpacity === undefined ? {} : { opacity: bodyOpacity }) }}>{text}</div>
  </div>
);
