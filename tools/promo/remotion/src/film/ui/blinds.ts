...
/** Le varianti del corto (Franz, 23/09 22:17). `linear`: le barre crescono a velocità costante fino a tutto schermo,
 *  senza l'accenno di voltata che le fermava. `flip`: la seconda metà si volta di 180° a righe alterne, col retro nero,
 *  e i listelli non spariscono: alla fine la tapparella è tutta nera. Senza, la tapparella del film lungo. */
export type BlindOpts = { linear?: boolean; flip?: boolean };
/** Il ritardo della voltata a strisce: le righe dispari partono dopo le pari, e tutte un filo dopo, a ventaglio dalle barre. */
const flipDelay = (i: number, n: number, bars: readonly number[]): number => (i % 2) * 0.1 + blindDelay(i, n, bars) * 0.08;
export const blindAt = (p: number, i: number, n: number, bars: readonly number[] = [6, 7], o: BlindOpts = {}): Blind => {
  ...
  const spread = isBar ? (o.linear ? clamp(p / 0.28) : ease(clamp(p / 0.28))) : 1;
  ...
  const wink = isBar && !o.linear ? Math.sin(Math.PI * clamp((p - 0.24) / 0.08)) : 0;
  ...
  const r = o.flip ? ease(clamp((p - 0.6 - flipDelay(i, n, bars)) / 0.22)) : ease(clamp((p - 0.6 - del * 0.18) / 0.26));
  const rot = o.flip ? -r * Math.PI : -r * (Math.PI / 2 + 0.22) - wink * 0.2;
  // il listello svanisce quando è quasi di taglio: l'uscita segue la voltata, non l'orologio
  const alpha = o.flip ? 1 : 1 - out(clamp((Math.abs(rot) - 1.35) / 0.2));
...
/** La larghezza di una barra che cresce: dalla lunghezza del suo dato (`fill` della traccia larga `track`) a `full`. */
export const barWidth = (spread: number, fill: number, track: number, full: number): number => fill * track + (full - fill * track) * spread;
