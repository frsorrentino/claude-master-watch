...
export const dotsAt = (p: number, len: number): Dots => {
  ...
  const bell = (i: number): number => {
    const d = Math.abs(b - -DOT_STEP * (3 - i)) / (DOT_STEP * 0.5);   // mezzo passo per lato: uno per volta, non sfumati l'uno nell'altro
  ...
  const lit = (i: number): number => Math.max(bell(i), b >= -DOT_STEP * (3 - i) ? DOT_HOLD : 0);
