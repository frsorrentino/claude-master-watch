...
export const UiTerminalPanel: React.FC<{ w: number; header: string; title: string; lines: string[]; shown: number; morph: number }> = ({ w, header, title, lines, shown, morph }) => {
  ...
  const bar = 44 * m, pad = 12 + 18 * m;
  return (
    <div style={{ width: w, boxSizing: "border-box", borderRadius: 14 * m, background: `rgba(30,34,41,${m})`, boxShadow: m > 0 ? `0 ${30 * m}px ${60 * m}px ${-10 * m}px rgba(4,5,12,${0.7 * m}), inset 0 0 0 ${1.5 * m}px rgba(235,244,255,${0.09 * m})` : undefined, overflow: "hidden", fontFamily: "Noto Sans Mono", color: UI.text }}>
      ...
      <div style={{ padding: `${8 + 10 * m}px ${pad}px ${10 + 14 * m}px` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, height: 34 * (1 - m), opacity: 1 - m, overflow: "hidden", fontFamily: "Roboto", fontWeight: 500, fontSize: 30, color: UI.briefGood }}>
        ...
        {lines.map((l, i) => {
          ...
          return (
            <div key={i} style={{ fontSize: 26, lineHeight: "36px", whiteSpace: "pre", opacity: e, translate: `0 ${(1 - e) * 10}px`, color: prompt ? UI.text : UI.text2, fontWeight: prompt ? 700 : 400 }}>
              {m > 0.5 && prompt ? <span style={{ color: UI.coral }}>❯ </span> : null}{prompt && m > 0.5 ? l.slice(2) : l}
            </div>
          );
        })}
        {/* il cursore del PC: lampeggia solo nella finestra */}
        <div style={{ height: 36 * m, opacity: m, fontSize: 26, lineHeight: "36px" }}><span style={{ color: UI.coral }}>❯ </span><span style={{ display: "inline-block", width: 14, height: 26, background: UI.text, verticalAlign: "-4px", opacity: Math.floor(shown * 3) % 2 === 0 ? 1 : 0.15 }} /></div>
      </div>
    </div>
  );
};
