...
import { CC, ccStackBottom } from "./claudeCode.ts";
import { UiClaudeCode, type CcRow } from "./UiClaudeCode.tsx";
import { UiCard } from "./UiCard.tsx";
import { LIST_BODY, UI } from "./UiTokens.ts";
import { slotRect, takeInAt, takeInFrontAt, takeInLineBelow, takeInRect, toDisplay } from "./takeIn.ts";
...
/** Le righe della card d'arrivo: l'esito del corto, «Released 2.8.0 and / tagged v2.8.0», ne occupa due nella riga della lista.
 *  UiCard va a capo col CSS, quindi il numero sta qui scritto: un esito di altra lunghezza va rimisurato sulla resa. */
const CARD_LINES = 2;
 ...
 */
const TakeInLayer: React.FC<Props & { line?: boolean }> = ({ e, prev, g, f, frames, dx, dy, u, width, height, line = true }) => {
  const term = (prev?.fx ?? []).find((x): x is TermFx => x.kind === "terminalPlane");
  if (!prev || !term || f < 0 || f >= frames) return null;
  const p = f / frames;
  const k = takeInAt(p);
  const r = takeInRect(slotRect(e.slot, dx, dy, u, CARD_LINES), p, { w: width, h: height });
  ...
  const y0 = ccStackBottom(height) - CC.row;
...
export const TakeInFrame: React.FC<Props> = (props) => <TakeInLayer {...props} line={takeInLineBelow(props.f / props.frames)} />;
...
export const TakeInScreen: React.FC<Props> = (props) => {
  ...
  return (
    <div style={{ position: "absolute", left: 0, top: 0, width: props.width, height: props.height, transformOrigin: "0 0",
      ...
      <TakeInLayer {...props} line={takeInLineBelow(props.f / props.frames)} />
    </div>
  );
};
