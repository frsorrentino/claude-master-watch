...
import { END_PACE, endColors } from "./endCard.ts";
import type { EndPace, EndTone } from "./endCard.ts";

export const REPO = "github.com/frsorrentino/claude-master-watch";

export const EndCard: React.FC<{ beat: number; pace?: EndPace; tone?: EndTone }> = ({ beat, pace = "normal", tone }) => {
  const P = END_PACE[pace];
  const C = endColors(tone);
  const f = useCurrentFrame();
  const o = (from: number) => interpolate(f, [from, from + 12], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const small: React.CSSProperties = { fontFamily: "Inter", fontWeight: 500, color: C.dim, textAlign: "center" };
  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", gap: 22, paddingTop: 330 }}>
      ...
      <div style={{ ...small, fontSize: THEME.service, color: C.title, opacity: o(beat * P.sub) }}>Free. <span style={{ color: C.accent }}>Open source.</span></div>
