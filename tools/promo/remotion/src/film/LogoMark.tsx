...
export const LogoMark: React.FC<{ draw: number; track?: string; ring?: boolean }> = ({ draw, track = "#3A404C", ring: ringOn = true }) => {
  ...
  return (
    <svg viewBox="0 0 480 480" style={{ position: "absolute", inset: 0 }}>
      {/* `ring` false: l'anello lo sta disegnando la forma unica (Shape.tsx), qui resta solo il «>_» */}
      {ringOn ? ring(sweep, track) : null}
      {ringOn && draw > 0.01 ? ring(sweep * 0.7 * draw, "#D97757") : null}
