...
export type WatchCue = { view: "front" | "threeQuarter" | "drawn" | "side"; column?: "right"; clip: string; clipStart?: number; rate?: number; freeze?: boolean; hold?: number; still?: string; steady?: boolean; exitBeats?: number; exitHold?: number; fadeOut?: number; enterAt?: number; enter?: Move; exit?: Move; camera?: Camera; dolly?: Dolly };   // `column`: «right» tiene l'orologio nella colonna di destra anche senza testo   // `fadeOut`: battiti di dissolvenza in coda; `enterAt`: a che battito della scena comincia l'entrata (prima non c'è)   // freeze: la clip resta ferma su clipStart (schermo fermo durante la lettura)   // `hold`: battiti di schermo fermo a inizio scena, poi il filmato parte (il display e i pannelli raccontano lo stesso punto)
