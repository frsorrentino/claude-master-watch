...
export const SceneView: React.FC<{ scene: Scene; overlay?: React.ReactNode; around?: React.ReactNode }> = ({ scene, overlay, around }) => {
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", inset: 0, opacity: (1 - underTakeover) * (1 - underFlip) * (scene.watch?.camera === "around" ? 1 - Math.max(0, (watchIn - 0.75) / 0.25) : 1) }}><Heroes scene={scene} prev={prev} g={GRID} watchCx={cx} pose={w?.view === "front" && pose ? { ...pose, scale: pose.scale * zoom } : null} glassPx={THEME.frontGlassPx} /></div>
  ...
  return (
    ...
    <AbsoluteFill style={{ background: "#000" }}>
      ...
      {TIMELINE.scenes.map((s, i) => {
        ...
        return <Sequence key={`blinds-${s.id}`} from={beatToFrame(GRID, next.at) - Math.round(frames * BLIND_CUT)} durationInFrames={frames + 1} layout="none"><Blinds frames={frames} to={s.blinds.to} bars={blindBars(((s.fx ?? []).find((e) => e.kind === "aside" && e.out === "bars") as { rows?: unknown[] } | undefined)?.rows?.length ?? 2)} /></Sequence>;
      })}
