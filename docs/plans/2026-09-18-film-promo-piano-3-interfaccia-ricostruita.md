...
| 3 onda della voce (speaks) | 1 giro: funziona come segno, non come momento forte (master: un filo sotto il testo, largo un quarto della colonna). **Giro 2 da fare** (metro Ask 77,5 s): l'onda nasce dal tasto ▶ sul display, attraversa il vetro e corre verso le parole, ampiezza 3-4 volte l'attuale sui picchi veri, larghezza tutta la colonna; le parole che si accendono restano | aae4842 | no; la voce vera arriverà dalla presa audio (piano 2 §3), rigenerare `question.env.json` con `envelope.py` |
